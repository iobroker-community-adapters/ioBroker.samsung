...
/**
 * Probe a host address to check if it's alive
 *
 * @param {string} addr - The address to probe
...
function probe(addr, config, callback) {
    ...
    var ls = null;
    // var log = config.log || console.log; // Unused variable
    var outstring = '';
    ...
    if (!init) {
        init = function (_ip) {
            ...
            if (xfamily.indexOf(p) !== -1) {
                ...
                if (config.timeout !== false) {
                    _args.push(`-w ${config.timeout * 1}` /*000*/);
                }
